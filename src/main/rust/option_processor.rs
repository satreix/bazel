...
pub struct OptionProcessor {
    ...
    //   blaze_exit_code::ExitCode ParseOptions(const std::vector<std::string>& args, const std::string& workspace, const std::string& cwd, std::string* error);
    //
...
//       system_bazelrc_path_(BAZEL_SYSTEM_BAZELRC_PATH) {}
    pub fn new(default_startup_options: StartupOptions) -> Self {
        Self{
            blazerc_and_env_command_args: Vec::<String>::new(),
            cmd_line: None,
...
//
//
// std::string OptionProcessor::get_lowercase_product_name() const {
//   return startup_options_->get_lowercase_product_name();
// }
//
...
//     blaze_exit_code::ExitCode parse_rcfile_exit_code = parse_rc_file(
...
// }
//
// blaze_exit_code::ExitCode parse_rc_file(const WorkspaceLayout* workspace_layout,
