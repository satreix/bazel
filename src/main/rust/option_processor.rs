...
const BAZEL_SYSTEM_BAZELRC_PATH: &str = "/etc/bazel.bazelrc";
#[cfg(target_family = "windows")]
const BAZEL_SYSTEM_BAZELRC_PATH: &str = "%%ProgramData%%/bazel.bazelrc";
...
impl OptionProcessor {
    pub fn new(logger: slog::Logger, default_startup_options: StartupOptions) -> Self {
        Self {
            logger,
            ...
            system_bazelrc_path: BAZEL_SYSTEM_BAZELRC_PATH,
        }
    }
    ...
    ///
    /// TODO(#4502) Consider simplifying result_rc_files to a vector of RcFiles, no unique_ptrs.
    fn get_rc_files(
        &self,
        workspace: &PathBuf,
        cwd: &PathBuf,
        ...
        //   const std::set<std::string> old_files = internal::GetOldRcPaths(workspace_layout, workspace, cwd, cmd_line->path_to_binary, cmd_line->startup_args, internal::FindSystemWideRc(system_bazelrc_path_));
        ...
        //     BAZEL_LOG(WARNING) << "The following rc files are no longer being read, please transfer their contents or import their path into one of the standard rc files:\n" << joined_lost_rcs;
        //   }
    ...
    pub fn parse_options(
        ...
        self.cmd_line = match self.split_command_line(args) {
            Ok(x) => Some(x),
        ...
        // exception.
        let mut rc_files = Vec::<RcFile>::new();
        if !bazel_util::search_nullary_option(&self.cmd_line.as_ref().unwrap().startup_args, "ignore_all_rc_files", false) {
            for i in self.get_rc_files(workspace, cwd, self.cmd_line.as_ref().unwrap())? {
                rc_files.push(i);
            }
        }
...
//        self.blazerc_and_env_command_args =
 //           self.get_blazerc_and_env_command_args(cwd, rc_file_ptrs, GetProcessedEnv());
    ...
    fn parse_startup_options(&mut self, rc_files: Vec<RcFile>) -> Result<(), exit_code::Error> {
        ...
        let mut rcstartup_flags = Vec::<RcStartupFlag>::new();
        for blazerc in rc_files {
        ...
        for arg in &self.cmd_line.as_ref().unwrap().startup_args {
            if !is_arg(&arg) {
                break
            }

            rcstartup_flags.push(RcStartupFlag::new("".to_string(), arg.to_string()));
        }

        self.startup_options.process_args(rcstartup_flags)
    }
    ...
    //   static constexpr const char* vars_to_uppercase[] = {"PATH", "SYSTEMROOT", "SYSTEMDRIVE", "TEMP", "TEMPDIR", "TMP"};
    //
    ...
    //   if (std::find(std::begin(vars_to_uppercase), std::end(vars_to_uppercase), name) != std::end(vars_to_uppercase)) {
    ...
    //     // ["c", "/foo"] so must use convert_path here. See GitHub issue #1684.
    //     env_str->assign("TMP=" + blaze_util::convert_path(env_str->substr(pos + 1)));
    //   }
    // }
    //
    ...
    fn get_blazerc_and_env_command_args(
        ...
        //
        //       result.push_back("--rc_source=" + blaze_util::convert_path(source_path));
        ...
        //   }
        //   result.push_back("--client_cwd=" + blaze_util::convert_path(cwd));

        result.push(format!(
            "--client_cwd={}",
            bazel_util::convert_path(cwd).to_str().unwrap()
        ));
        result
    }
    ...
    pub fn get_command_arguments(&self) -> Vec<String> {
        ...
        if self.cmd_line.as_ref().unwrap().command_args.is_empty() {
            return Vec::new();
        }
